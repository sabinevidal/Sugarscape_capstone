...
architecture = ARGS[1]
scenario = "movement_reproduction_combat"
n_steps = 150
seed = 28
...
model = if architecture == "rule"
    sugarscape(; seed=seed, enable_reproduction=true, enable_combat=true, llm_metadata=llm_metadata, run_name=run_name)
elseif architecture == "llm"
    sugarscape(; seed=seed, enable_reproduction=true, enable_combat=true, use_llm_decisions=true, llm_metadata=llm_metadata, run_name=run_name)
elseif architecture == "bigfive"
    sugarscape_llm_bigfive(; seed=seed, enable_reproduction=true, enable_combat=true, llm_metadata=llm_metadata, run_name=run_name)
elseif architecture == "schwartz"
    sugarscape_llm_schwartz(; seed=seed, enable_reproduction=true, enable_combat=true, llm_metadata=llm_metadata, run_name=run_name)
else
...
mdata = reproduction_combat_metrics
adata = if architecture == "bigfive"
    ...
    [
        ...
        :last_partner_id, :last_credit_partner, :last_combat_partner, :chose_not_to_attack, :chose_not_to_reproduce, :traits
    ]
elseif architecture == "schwartz"
    ...
    [
        :pos, :sugar, :age, :vision, :metabolism, :sex,
        :children, :has_reproduced, :total_inheritance_received,
        :last_partner_id, :last_credit_partner, :last_combat_partner, :chose_not_to_attack, :chose_not_to_reproduce, :schwartz_values
    ]
else
