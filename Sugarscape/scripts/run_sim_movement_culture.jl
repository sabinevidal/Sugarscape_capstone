...
model = if architecture == "rule"
    sugarscape(; seed=seed, enable_culture=true, llm_metadata=llm_metadata, run_name=run_name)
elseif architecture == "llm"
    sugarscape(; seed=seed, enable_culture=true, use_llm_decisions=true, llm_metadata=llm_metadata, run_name=run_name)
elseif architecture == "bigfive"
    sugarscape_llm_bigfive(; seed=seed, enable_culture=true, llm_metadata=llm_metadata, run_name=run_name)
elseif architecture == "schwartz"
    sugarscape_llm_schwartz(; seed=seed, enable_culture=true, llm_metadata=llm_metadata, run_name=run_name)
else
...
adata = if architecture == "bigfive"
    ...
    [
        ...
        :last_partner_id, :last_credit_partner, :chose_not_to_spread_culture, :traits
    ]
elseif architecture == "schwartz"
    ...
    [
        :pos, :sugar, :age, :vision, :metabolism, :sex,
        :culture, :children, :has_reproduced, :has_spread_culture, :has_accepted_culture, :total_inheritance_received,
        :last_partner_id, :last_credit_partner, :chose_not_to_spread_culture, :schwartz_values
    ]
else
