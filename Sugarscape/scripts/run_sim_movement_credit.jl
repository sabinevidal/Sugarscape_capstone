...
model = if architecture == "rule"
    sugarscape(; seed=seed, enable_credit=true, llm_metadata=llm_metadata, run_name=run_name)
elseif architecture == "llm"
    sugarscape(; seed=seed, enable_credit=true, use_llm_decisions=true, llm_metadata=llm_metadata, run_name=run_name)
elseif architecture == "bigfive"
    sugarscape_llm_bigfive(; seed=seed, enable_credit=true, llm_metadata=llm_metadata, run_name=run_name)
elseif architecture == "schwartz"
    sugarscape_llm_schwartz(; seed=seed, enable_credit=true, llm_metadata=llm_metadata, run_name=run_name)
else
...
adata = if architecture == "bigfive"
    ...
    [
        ...
        :last_partner_id, :last_credit_partner, :chose_not_to_borrow, :chose_not_to_lend, :traits
    ]
elseif architecture == "schwartz"
    ...
    [
        :pos, :sugar, :age, :vision, :metabolism, :sex,
        :children, :loans_given, :loans_owed, :has_reproduced, :total_inheritance_received,
        :last_partner_id, :last_credit_partner, :chose_not_to_borrow, :chose_not_to_lend, :schwartz_values
    ]
else
