...
model = if architecture == "rule"
    sugarscape(; seed=seed, enable_credit=true)
elseif architecture == "llm"
    sugarscape(; seed=seed, enable_credit=true, use_llm_decisions=true)
elseif architecture == "bigfive"
    sugarscape_llm_bigfive(; seed=seed, enable_credit=true)
elseif architecture == "schwartz"
    sugarscape_llm_schwartz(; seed=seed, enable_credit=true)
else
